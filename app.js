...
async function playSong(id) {
    ...
    // Find index based on current rendered sorted list
...
document.getElementById('btn-export').onclick = async () => {
    ...
    try {
        ...
        document.getElementById('backup-textarea').value = JSON.stringify(backup);
...
document.getElementById('btn-copy-backup').onclick = () => {
    ...
    }).catch(() => alert("Kopyalama başarısız oldu. Manuel kopyalayın."));
};
...
document.getElementById('btn-confirm-text-restore').onclick = async () => {
    ...
    try {
        ...
        console.error(err); 
        alert("Geri yükleme hatası. Metnin doğru ve eksiksiz olduğundan emin olun.");
        el.viewTitle.innerText = currentView === 'all' ? "Tüm Şarkılar" : el.viewTitle.innerText;
    }
};
